...
int Config_ScanWlanExt(HTTPCONNECTION hConnection, LIST *pParamList, int iAction, XML *pReturnXML)
{
	ApScanResult ScanAp[32];
	int nLen = 0;
	int i;
	...
	case CA_CONFIG:
		...
		{ // get ap information
			for(i = 0; i < nLen; i++)
			{
				...
				sprintf(macAdresse, "%02X:%02X:%02X:%02X:%02X:%02X",
				  ScanAp[i].mac[0], ScanAp[i].mac[1],
				  ScanAp[i].mac[2], ScanAp[i].mac[3],
				  ScanAp[i].mac[4], ScanAp[i].mac[5]);

				
				AddHttpValue(pReturnXML, G_PC_MAC  ,macAdresse);
